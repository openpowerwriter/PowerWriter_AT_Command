	...
	E_ATCmdType cmd)
{
	...
	case ATCmdSwitchOfflineProject:
		return sizeof(S_ATCmdSwitchOfflineProject); // Switch offline project
...
/*
 * @brief switch powerwriter offline project
 * @pram 	ch: AT channel object
 ...
 * @return  Returns true on success, false otherwise
 */
bool powerwriter_at_project_switch(
	S_ATChannel *ch,
	S_SwitchPrjReloadType reload_type,
	const char *prj_name,
	const char *password)
{
	AT_CHECK_PARAM(ch, false)
	...
	/* start signal */
	if (strlen(password) >
		sizeof(ch->m_cmdOutput.m_payload.m_cmdProperty.m_ATCmdSwitchOfflineProject.m_password))
	{
		_cmdSetLastError(ch, ATErrorKey);
		return false;
	}
	// reload type
	ch->m_cmdOutput.m_payload.m_cmdProperty.m_ATCmdSwitchOfflineProject.m_reloadType = reload_type;

	// name
	memset(&ch->m_cmdOutput.m_payload.m_cmdProperty.m_ATCmdSwitchOfflineProject.m_name, 0,
		sizeof(ch->m_cmdOutput.m_payload.m_cmdProperty.m_ATCmdSwitchOfflineProject.m_name));
	strncpy(ch->m_cmdOutput.m_payload.m_cmdProperty.m_ATCmdSwitchOfflineProject.m_name, prj_name,
		MIN(sizeof(ch->m_cmdOutput.m_payload.m_cmdProperty.m_ATCmdSwitchOfflineProject.m_name) - 1, strlen(prj_name)));

	// password
	memset(&ch->m_cmdOutput.m_payload.m_cmdProperty.m_ATCmdSwitchOfflineProject.m_password, PW_AT_DEFAULT_PASSWORD,
		   sizeof(ch->m_cmdOutput.m_payload.m_cmdProperty.m_ATCmdSwitchOfflineProject.m_password));
	memcpy(ch->m_cmdOutput.m_payload.m_cmdProperty.m_ATCmdSwitchOfflineProject.m_password, password, strlen(password));
	
	return _powerwriter_at_send_command(ch, ATCmdSwitchOfflineProject, PW_AT_TIMEOUT_BASE *3);
}
