...
public:
	...
	virtual int readData(std::vector<char> & data, int maxSize);
    /**
     ...
     * @retval [other] return number Of bytes read 返回读取字节数
     */
    virtual int readAllData(std::vector<char> & data);
    /**
...
public:
   ...
   sigslot::signal0<> disconnectCallback;	///< sigslot for disconnect 串口异常断开
