        ...
        while (p_base->isThreadRunning())
        {
            ...
            if (WaitForSingleObject(m_overlapMonitor.hEvent, INFINITE) == WAIT_OBJECT_0)
            {
                ...
                if (!GetOverlappedResult(m_mainHandle, &m_overlapMonitor, &undefined, false))
                {
					...
					p_base->disconnectCallback._emit();
                    return 1; //@ todo
                }
...
int CSerialPortWinBase::readData(std::vector<char> & data, int maxSize)
{
	DWORD dRet = 0;
	...
	if (isOpened())
	{
		if (m_operateMode == itas109::/*OperateMode::*/ AsynchronousOperate)
		{
			...
			if (ReadFile(m_handle, (void *)data.data(), (DWORD)maxSize, &dRet, &m_overlapRead))
			{
				// data[dRet] = '\0';
			}
			else
		...
		{
			if (ReadFile(m_handle, (void *)data.data(), (DWORD)maxSize, &dRet, NULL))
			{
			}
			else
			{
...
int CSerialPortWinBase::readAllData(std::vector<char> & data)
{
    int maxSize = 0;
	...
	data.resize(maxSize + 1);	//last for \0
